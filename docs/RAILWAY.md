...
| `PLANNER_FPL_ENTRY_ID` | Your FPL entry ID — personal squad planner on `/planner` (Samuel: `3386632`) |
| `PORT` | `3000` |
...
| sync-cron "Deployment crashed" | Railway treats non-zero exits as crashes — job now always exits 0 after closing the DB pool; check deploy logs for import/enrich errors |
